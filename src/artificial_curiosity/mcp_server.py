"""Stdlib MCP server for Artificial Curiosity (JSON-RPC 2.0 over stdio).
    ...
    if method == "initialize":
        return _ok(
            id_,
            {
                ...
                "instructions": (
                    "Artificial Curiosity ranks valuable *unanswered* questions. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] in ("-h", "--help"):
        print(
            "Artificial Curiosity MCP server (stdio).\n"
            "  curiosity-mcp\n"
            "  python -m artificial_curiosity.mcp_server\n\n"
