...
1. Optional `critique_brief` tool — returns form issues only; does not change ranks silently.  
   **WIP (sibling):** `src/artificial_emotions/critique.py` — regex form critic (sprawl, missing falsifier, anthropomorphism, citation tells). Wire to MCP/API when ready; keep “does not change ranks.”  
